...
def inputs(blocs: list[dict[str, str | list[list[str]]]]) -> tuple[str, int, int]:
    inputed_number = ""
...
if __name__ == "__main__":
    ...
    size: tuple[int, int] = Grid.get_size(grid_matrice)
    ...
    while True:
        ...
        inputed_string, x_index, y_index = inputs(usable_blocs)
        if not Grid.valid_position(grid_matrice, usable_blocs[int(inputed_string)-1]["matrice"], x_index, y_index):
            ...
            if References.try_pos >= 3:
                game.stop()
            continue
        Grid.emplace_bloc(grid_matrice, usable_blocs[int(inputed_string)-1]["matrice"], x_index, y_index)
        game.clear_notification()
